...
def get_panel(panel_name, user = ''):
    ...
    if panel_name == "email.queue":
        data = mail_queue()
        ppanel['tbl_source']['table'] = data
    if panel_name == "email.filterlists":
        dataw = get_whitelist()
        datab = get_blacklist()
        ppanel['tbl_source']['tablew'] = dataw
        ppanel['tbl_source']['tableb'] = datab
